...
func TestOpenAIProvider_Embed(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for i := range req.Input {
			...
			resp.Data[i].Embedding = make([]float32, 768)
			for j := range resp.Data[i].Embedding {
				resp.Data[i].Embedding[j] = float32(i+j) * 0.001
			}
		}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := embeddingResponse{
			...
			Data: []struct {
				...
				{Index: 0, Embedding: []float32{3, 4}}, // L2 norm = 5, normalized = [0.6, 0.8]
			},
		}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		resp := embeddingResponse{
			...
			Data: []struct {
				...
				{Index: 0, Embedding: make([]float32, 768)},
			},
		}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
